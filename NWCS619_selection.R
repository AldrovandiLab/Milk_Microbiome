...
#####################################################################################################
### ADDITIONAL 10/23/2019: extract ARV regimens
#mapping <- read.table("/Lab_Share/PROMISE/nwcs619/nwcs619_Mapping.083019.txt", header=T, as.is=T, sep="\t")
#arv <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/ARV_REGIMEN_PROMISE_WITH_HOLDS.csv")
#arv <- subset(arv, patid %in% mapping$patid)
#write.table(arv, file="/Lab_Share/PROMISE/nwcs619/20181207/ARV.102319.txt", quote=F, sep="\t", row.names=F, col.names=T)


####################################################################################################
## ADDITIONAL 11/05/2019: find off-ARV subjects for R01
mpatids <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/MPATIDS.csv", row.names=1)
# load gestational age and delivery data
data <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/ARV_START.csv")
rownames(data) <- data$patid
...
# load ARV data
arv <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/ARV_REGIMEN_PROMISE_WITH_HOLDS.csv")
for (mvar in c("regimen_start", "regimen_stopdate", "deldt1")) {
	arv[,mvar] <- as.Date(arv[,mvar], format="%d%B%Y")
}
# for each period of off-ARV, find matched gestational ages
arv$GestationalAgeAtRegimenStart <- data[as.character(arv$patid), "delgage"] + as.numeric(((arv$regimen_start - arv$deldt1)/7))
arv$GestationalAgeAtRegimenStop <- pmin(data[as.character(arv$patid), "delgage"] + as.numeric(((arv$regimen_stopdate - arv$deldt1)/7)), data[as.character(arv$patid), "delgage"])
arv$RegimenLengthInDays <- pmin(as.numeric(arv$regimen_stopdate - arv$regimen_start), as.numeric(arv$deldt1 - arv$regimen_start))
arv$delgage <- data[as.character(arv$patid), "delgage"]
arv.off <- subset(arv, regimen=="no ARVs" & regimen_start < deldt1) # subset to intervals off-ARV and started prior to delivery
out <- arv.off[, c("regimen", "regimenD", "deldt1", "regimen_start", "regimen_stopdate", "patid", "GestationalAgeAtRegimenStart", "GestationalAgeAtRegimenStop", "RegimenLengthInDays", "delgage")]
write.table(out, file="/Lab_Share/PROMISE/nwcs619/20181207/off_ARV_intervals.110519.txt", quote=F, sep="\t", row.names=F, col.names=T)

## ADDITIONAL 11/15/2019: look at LBW/PTB (birthweight and gestational age)
mpatids <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/MPATIDS.csv", row.names=1)
# load gestational age and delivery data
data <- read.csv("/Lab_Share/PROMISE/nwcs619/20181207/ARV_START.csv")
rownames(data) <- data$patid
...
for (mvar in c("gender", "weight0week", "weight1week")) {
	data[, mvar] <- infant_baseline[as.character(data$cpatid), mvar]
}
data$LBW <- ifelse(data$weight0week < 2500, "LBW", "NotLBW"); data$LBW <- factor(data$LBW, levels=c("NotLBW", "LBW"))
data$PTB <- ifelse(data$delgage < 37, "Preterm", "Term"); data$PTB <- factor(data$PTB, levels=c("Term", "Preterm"))
table(data[,c("PTB", "LBW")])

pdf("/Lab_Share/PROMISE/nwcs619/metabolon/Birthweight_by_GA.pdf", width=12)
p <- ggplot(data, aes(x=delgage, y=weight0week, color=PTB)) + geom_point() + theme_classic() + ggtitle(sprintf("Birthweight by gestational age")) + scale_color_brewer(palette="Set1") + geom_hline(yintercept=2500) + geom_vline(xintercept=37) + stat_smooth(method="lm")
print(p)
# distribution of gestational age stratified by LBW
p <- ggplot(subset(data, PTB=="Term" & delgage <= 42), aes(x=delgage, fill=LBW)) + geom_histogram(position=position_dodge()) + theme_classic() + ggtitle(sprintf("Distribution of gestational age by LBW (truncate at 42wks)")) + scale_fill_brewer(palette="Set1")
print(p)
